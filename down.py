...
tms["seller_id"] = []
no=0
video = "BAACAgUAAxkBAAIOOmcHwOBwY6K9eaw3Qz31vjPI6WrxAAIdEQACVclBVNEI5Y6GDskYNgQ"
channels = ['@testing_b_gc']
BROADCAST_MESSAGE = 1
ADD_VIDEO = 1
def start(update: Update, context: CallbackContext):
    ...
    # If the user is not a member, exit the function
            if member.status not in ['member', 'creator', 'administrator']:
                download_link = f"https://t.me/IHGtradegroup"
                download_linker = f"https://t.me/IHG_Hexa_Auction"
            ...
            # Send the download link with inline keyboard
                ...
                context.bot.send_video(chat_id=update.message.chat.id,video=video,caption=f"""♦𝑊𝑒𝑙𝑐𝑜𝑚𝑒 {update.message.from_user.name}
...
<blockquote>𝘾𝙧𝙚𝙖𝙩𝙤𝙧 :</blockquote> @l0_Mr_unknown_0l""",reply_markup=keyboard, parse_mode="html")
    ...
    if collection_id:
        ...
        context.bot.send_video(chat_id=update.message.chat.id,video=video,caption=f"""𝙒𝙚𝙡𝙘𝙤𝙢𝙚 {update.message.from_user.name} , 
🅃🄾 🄰🄳🄳 🄸🅃🄴🄼🅂 🄵🄾🅁 🄰🅄🄲🅃🄸🄾🄽 🅄🅂🄴 /add
<blockquote>𝘾𝙧𝙚𝙖𝙩𝙤𝙧 :</blockquote> @l0_Mr_unknown_0l""",reply_markup=keyboard, parse_mode="html")
  ...
  if query:
    if query.data.startswith("confirm_sell_"):
      ...
      if query.from_user.id in approved_users:
        ...
        trade_group_link_hidden = "𝑇𝑅𝐴𝐷𝐸 𝐺𝑅𝑂𝑈𝑃 "
        ...
        # Send the edited message with the reply_markup
...
> BASE PRICE: {base_price}""",parse_mode="markdown")
    ...
    # Edit the original message caption
        ...
        if category == "legendary":
            ...
            legendary_item.append(f"<a href='https://t.me/{AUC_NAME}/{msid}'>{item_name}</a>") 
        elif category == "non-legendary":
            if seller_id in seller_lead:
            ...
            non_legendary_item.append(f"<a href='https://t.me/{AUC_NAME}/{msid}'>{item_name}</a>") 
        elif category == "shiny":
            if seller_id in seller_lead:
            ...
            shiny_item.append(f"<a href='https://t.me/{AUC_NAME}/{msid}'>{item_name}</a>") 
    if callback_data.startswith('tapprove'):
        ...
        mm=context.bot.send_message(chat_id=AUCTION_GROUP_ID, text=f" > {category.upper()} NAME: {item_name}\n\n{category.upper()} DETAILS:\n{details}\n\n > BASE PRICE: {base_price}",parse_mode="markdown")
...
def items(update: Update, context: CallbackContext):
  user_id = update.effective_user.id # Use effective_user for inline queries

  # Check if called via inline query
  if update.callback_query:
    query = update.callback_query
    data = query.data

    # Update based on inline button click
    if data == "item_legendary":
      message_text = "\n".join(legendary_item) # Update text with legendary items
      keyboard = InlineKeyboardMarkup([[
        InlineKeyboardButton("Non-Legendary", callback_data="item_non_legendary"),
        InlineKeyboardButton("Shiny", callback_data="item_shiny"),
        InlineKeyboardButton("Team", callback_data="item_team"),
        InlineKeyboardButton("TM", callback_data="item_tm"),
      ]])
    elif data == "item_non_legendary":
      message_text = "\n".join(non_legendary_item) # Update text with non-legendary items
      keyboard = InlineKeyboardMarkup([[
        InlineKeyboardButton("Legendary", callback_data="item_legendary"),
        InlineKeyboardButton("Shiny", callback_data="item_shiny"),
        InlineKeyboardButton("Team", callback_data="item_team"),
        InlineKeyboardButton("TM", callback_data="item_tm"),
      ]])
    elif data == "item_shiny":
      message_text = "\n".join(shiny_list) # Update text with shiny items
      keyboard = InlineKeyboardMarkup([[
        InlineKeyboardButton("Legendary", callback_data="item_legendary"),
        InlineKeyboardButton("Non-Legendary", callback_data="item_non_legendary"),
        InlineKeyboardButton("Team", callback_data="item_team"),
      ...
      message_text = "\n".join(team_item) # Update text with team items
      keyboard = InlineKeyboardMarkup([[
        InlineKeyboardButton("Legendary", callback_data="item_legendary"),
        InlineKeyboardButton("Non-Legendary", callback_data="item_non_legendary"),
        InlineKeyboardButton("Shiny", callback_data="item_shiny"),
      ...
      message_text = "\n".join(tm_item) # Update text with TM items
      keyboard = InlineKeyboardMarkup([[
        InlineKeyboardButton("Legendary", callback_data="item_legendary"),
        InlineKeyboardButton("Non-Legendary", callback_data="item_non_legendary"),
        InlineKeyboardButton("Shiny", callback_data="item_shiny"),
      ...
      keyboard = None
    query.edit_message_text(text=f"HERE IS THE LIST OF ITEMS \n{message_text}", parse_mode="html", disable_web_page_preview=True, reply_markup=keyboard)
  else:
    message_text = "HERE IS THE LIST OF ITEMS"
    ...
    context.bot.send_message(chat_id=update.message.chat.id, text=f"{message_text}",reply_markup=keyboard,disable_web_page_preview=True, parse_mode="html")
